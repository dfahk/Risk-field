...
lambda_value = 1.0
mu = 0.05
gamma_param = 0.03  # 为避免与math.gamma冲突，这里用 gamma_param

# 车辆参数配置（注意：车辆位置需落在道路风险场网格内）
vehicles = [
    {
        'x_i': 1560,       # 车辆1质心全局坐标x
        'y_i': 3954.8,       # 车辆1质心全局坐标y
        'theta_i': np.deg2rad(0),   # 车辆1航向角
        'L': 4.8,         # 车长
        'W': 1.8,         # 车宽
        'a_i': 3,         # 加速度
        'v_i': 15,        # 速度
    ...
    {
        'x_i': 1570,      # 车辆2质心全局坐标x
        'y_i': 3954.8,      # 车辆2质心全局坐标y
        'theta_i': np.deg2rad(0),   # 车辆2航向角
        'L': 4.8,
        'W': 1.8,
        'a_i': 2,
        'v_i': 10,
...
# ---------------------- 综合风险场（道路 + 车辆） ---------------------- #
# 对于道路风险场中未计算（nan）的部分视为0风险
Z_combined = np.nan_to_num(Z_road, nan=0) + Z_vehicle
...
for idx, vehicle in enumerate(vehicles):
    ...
    body_line, = ax.plot(rotated_corners[:,0], rotated_corners[:,1],
    ...
    vel_arrow = ax.arrow(vehicle['x_i'], vehicle['y_i'], vx, vy,
    ...
    acc_arrow = ax.arrow(vehicle['x_i'], vehicle['y_i'], ax_a, ay_a,
                         color='red', width=0.05, linestyle='--')
    if idx == 0:
...
for idx, vehicle in enumerate(vehicles):
    ...
    text = f'Vehicle {idx+1}: Speed: {speed} m/s, Acceleration: {acceleration} m/s², Heading: {heading_angle:.2f}°'
    ax.text(0.5, 1.05 - idx * 0.05, text, transform=ax.transAxes, ha='center', va='bottom')

ax.set_xlabel('World X Coordinate')
ax.set_ylabel('World Y Coordinate')
ax.set_title('Combined Road and Two-Vehicle Risk Field')
plt.tight_layout()
plt.show()
