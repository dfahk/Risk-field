...
def E_i(x, y, xi, yi, m_i, v_i, a_i, l, w, alpha, beta, lambda_value, delta_1, delta_2, gamma_1, gamma_2, mu):
    ...
    term2 = np.sqrt(delta_1 * ((x - xi) / l ) ** 2 + delta_2 * ((y - yi) / w ) ** 2) / np.exp(gamma_2 * v_i*np.cos(eta))
    term3 = alpha * np.exp((-beta * np.linalg.norm(v_rel)**2) / (np.dot(P_rel, v_rel)))  # 修改这里
    # 最终的公式结果
